...
public class CardProcFrame extends AbstractFrame {
    ...
    private static Log log = LogFactory.getLog(CardProcFrame.class);
    ...
    private void initProc() {
        ...
        cardProcDs.addListener(new CollectionDsListenerAdapter<CardProc>() {
            @Override
            public void itemChanged(Datasource<CardProc> ds, @Nullable CardProc prevItem, @Nullable CardProc item) {
                if ((item != null) && (item.getStartCount() < 1)) {
    ...
    public void startProcess(final CardProc cp) {
        ...
        if (!PersistenceHelper.isNew(card)) {
            ...
            if (loadedCard == null) {
                ...
                windowManagerProvider.get().close(window);
                return;
            }
        ...
        if (window instanceof Window.Editor && ((Window.Editor) window).commit()) {
            ...
            try {
                ...
                log.error(msg + ": " + ExceptionUtils.getStackTrace(e));
                String message;
